...
def update_pie_chart(selected_site):
    if selected_site == 'ALL':
        ...
        total_failed = spacex_df[spacex_df['class'] == 0].shape[0]
        fig = px.pie(names=['Success', 'Failure'], values=[total_success, total_failed],
                     title='Total Launch Success Rate')
    else:
        site_data = spacex_df[spacex_df['Launch Site'] == selected_site]
        site_success = site_data[site_data['class'] == 1].shape[0]
        site_failed = site_data[site_data['class'] == 0].shape[0]
        fig = px.pie(names=['Success', 'Failure'], values=[site_success, site_failed],
                     title=f'{selected_site} Launch Success Rate')
    return fig
# TASK 4:
...
def update_scatter_chart(selected_site, payload_range):
    ...
    fig.update_yaxes(title='Outcome (class)')
    return fig
